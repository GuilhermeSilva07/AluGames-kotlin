...
class PlanosDAO(manager: EntityManager): DAO<Plano, PlanoEntity>(manager, PlanoEntity::class.java) {
    ...
    override fun toEntity(plano: Plano): PlanoEntity {
        return if (plano is PlanoAssinatura){
            PlanoEntity.PlanoAssinaturaEntity(plano.tipo, plano.mensalidade, plano.jogosIncluidos, plano.percentualDescontoReputacao, plano.id)
        } else {
            PlanoEntity.PlanoAvulsoEntity(plano.tipo, plano.id)
        }
    }

    override fun toModel(entity: PlanoEntity): Plano {

        return if (entity is PlanoEntity.PlanoAssinaturaEntity) {
